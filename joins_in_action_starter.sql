...
SELECT lead_studio, COUNT(favorite_movie_id) AS Total_favorites FROM movies LEFT JOIN users ON movie_id = favorite_movie_id
...
SELECT AVG(rotten_tomatoes) AS Avg_rotten_tomato_score from movies JOIN users ON movie_id = favorite_movie_id;
